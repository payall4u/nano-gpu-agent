...
type QGPUManager interface {
	Run()
	GC()
	Restore() error
}

type QGPUManagerImpl struct {
	nodeName string
	dbPath   string
	sitter   kube.Sitter
...
type Option func(manager *QGPUManagerImpl)

func WithNodeName(nodeName string) Option {
	return func(manager *QGPUManagerImpl) {
...
func NewQGPUManager(options ...Option) (QGPUManager, error) {
	m := &QGPUManagerImpl{
...
func (m *QGPUManagerImpl) Run() {
	go m.sitter.Start()
	go m.gc()
	go m.plugin.Run(m.stopChan)
}

func (m *QGPUManagerImpl) Restore() error {
